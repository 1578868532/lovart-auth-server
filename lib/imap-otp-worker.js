...
function createImapOtpWorker({
  ...
  async function connectAndOpen() {
    ...
    clientHandlers = {
      exists: () => {
        markEvent();
        queueScan(getMinimumPendingBaseline()).catch(error => reportFailure(error, nextClient));
      },
  ...
  function queueScan(baselineUid) {
    if (activeScan) return activeScan;
    activeScan = scanNow(baselineUid).finally(() => {
      activeScan = null;
    });
    return activeScan;
  }
  ...
  async function stop() {
    ...
    connected = false;
    detachClient(clientToClose);
    if (clientToClose && !loggedOutClients.has(clientToClose)) {
      loggedOutClients.add(clientToClose);
      await clientToClose.logout();
    }
    client = null;
    stopping = false;
  }
